...
* **対象年月ヘッダの乗っ取り防止**：走査範囲を先頭2行に絞り、範囲内に**異なる年月が複数マッチしたら「曖昧」として throw**（fail-loud）。注記に紛れた別月（`1999年1月`）が全件を汚染するのを防ぐ。
* **ヘッダ年月: 全マッチ走査 + フル日付除外 + 複数年月は曖昧throw**：`MONTH_HEADER_RE` を**グローバル(g)で全マッチ走査**し、1セル内に複数年月（例 `1999年1月実績比較 2026年8月度`）があっても先頭だけ拾って黙って化ける silent hijack を防ぐ。加えて否定先読み `(?!\s*\d{1,2}\s*日)` で**フル日付形式（`2026年07月25日` 等の出力日時）を年月候補から除外**し、出力日時を第2の年月と誤認して正当シートを誤throwする DoS を防ぐ。`2026年8月`（単独）・`2026年8月度 シフト表`（後続語つき）はいずれも `2026-08` に解決。
