...
* **時刻列の採否判定**：出勤/退勤(列5/6)が「HH:MM 形式かつ 00:00 でない」行だけをシフトとして採用する。設計の元条件は `00:00` 除外のみだったが、時刻列に `休`/`遅刻` 等の非時刻文字列や空セルが混入した場合に空時刻コマを生成しないよう、HH:MM 形式チェックを併用する（`isWorkingTime`）。片方のみ 00:00 は除外、終了<開始の深夜跨ぎは同日 verbatim 記録（社長確定 v1）。
* **staffCode は規定位置固定 + fail-loud（推測しない）**：staffCode は唯一の安定キーで、誤ると別人の予定になる最悪事故に直結する。よって規定位置（4行目 col2）からのみ取得し、書式外/空なら**明示的に throw して取込中止**する。他行からの推測フォールバック（旧: 先頭12行走査）は**廃止**した（承認者等の他人コードを黙って本人コードと誤採用しうるため）。氏名(staffName)は表示用でキーではないため sheetName フォールバックを許容する。
* **staffCode の書式は英字1+数字ちょうど4桁**（`/^[A-Za-z]\d{4}$/`、社長確認で確定）。部署/等級コード（`A1`,`X99` 等）や桁数の異なる想定外コードを弾く。上限なし（`\d{4,}`）は誤同定余地が残るため4桁固定にした。
* **対象年月ヘッダ: 固定セル(rows[0][0])方式・他セル非走査・無ければ fail-loud throw**：対象年月は `targetMonth`（apps/web はファイル名から渡す本筋）を最優先し、無ければ**先頭セル `rows[0][0]`（タイトルセル）内の最初の `YYYY年M月` のみ**を採用する。**他のセル・他の行は一切見ない**。先頭セルに年月が無ければ throw。実xlsxは `rows[0][0] = "2026年8月"`（単独）で、`度`や後続語 `シフト表` があっても最初のマッチが本物の月。
  * 旧方式（緩い正規表現 + 先頭2行走査 + `/g` 全マッチ + 否定先読み + 複数年月は曖昧throw）は**廃止**。走査で候補を拾う限り別セル/別行からの silent hijack や、出力日時（`2026年07月25日`）を第2年月と誤認する誤throw の抜け道が残るため、「拾う候補が無ければ hijack も起きない」固定セル方式へアプローチごと差し替えた。データ領域・2行目・別セルに別月（`2025年12月` 等）が紛れても `rows[0][0]` の月に固定される。
  * **先頭アンカー（silent → fail-loud）**：`MONTH_HEADER_RE = /^\s*(\d{4})年\s*(\d{1,2})月/`。`rows[0][0]` は `normalizeText` 後、**先頭（先行空白のみ許容）が年月で始まる時だけ**採用し、始まらなければ throw。これにより「対象月の前に別年月が前置される」ケース（`出力日時 2026年07月25日 …`→2026-07、`前年比 2025年12月 実績 …`→2025-12）で黙って別月に化ける silent corruption を防ぐ。全角先頭空白 `　2026年8月` は normalizeText で半角化されアンカーを通過する。
  * **セル値の型崩れ耐性**：`normalizeText` は入口で `String(input ?? "")` に正規化し、身元行(`resolveIdentity`)のセル取得も共通ヘルパー `toCell(value) = String(value ?? "").trim()` に統一した。xlsxライブラリは数値/日付/真偽値を number/Date/boolean 型で返すため、氏名/コード/所属や `rows[0][0]` が非文字列でも `.trim is not a function` / `.replace is not a function` でパース全体がクラッシュせず、クリーンに throw または該当行スキップに落ちる（Phase2 結合時に効く防御）。文字列入力の挙動は不変。
  * **【既知の限界】残穴A（regexでは直さない）**：in-sheet の対象月検出は `rows[0][0]` が**正しい年月で始まる前提**。タイトルセルが誤った年月で始まる異常帳票（例 `2025年12月度からの繰越 2026年8月度` → 2025-12）では silent に誤月を採用しうる。これは in-sheet ヒューリスティックの原理的限界で、regex を足すと別の穴が開く堂々巡りになるためコードでは対処しない（実データ `2026年8月度…` では非発火）。
  * **【恒久策 / Phase2 必須】**：apps/web は**ファイル名（例 `<氏名>(<コード>) 2026年08月度.xlsx`）から `targetMonth` を導出して必ず渡す**こと。これにより in-sheet 検出はフォールバックへ格下げされ、残穴A は構造的に解消する。パッケージの `parseJobcanSheet` は `targetMonth` 指定時はヘッダ（`rows[0][0]`）を読まない（実装済み）。
* **日付セルの範囲検証 + 実在日検証**：月 1-12・日 1-31 の範囲外はスキップ。加えて**年補完後に暦上の実在日か逆照合**（`new Date(y,m-1,d)` で y/m/d 一致・うるう年考慮）し、`2/30`・非うるう年 `2/29` 等の非実在日をスキップする。下流の `new Date()` が非実在日を `3/2` 等へ黙って繰り上げるのを防ぐため。取込はバッチなので throw ではなく該当行スキップ。
* **日付セルの桁こぼれ防止**：日付正規表現に末尾境界（`(?=\D|$)`）を付け、`8/123`→`8/12` の桁こぼれを防ぐ（境界を満たさない行は日付行と見なさずスキップ）。
* **時刻の範囲検証**：出勤/退勤は HH:MM 書式に加え **時 0-23・分 0-59** を検証。書式のみだと `24:00`/`25:70`/`23:60`/`09:99` が通り不正時刻でカレンダーを汚すため。範囲外は「シフト時刻でない」として不採用（throw はしない）。
* **時刻正規化の対称性**：`normalizeTime` は時・分の双方をゼロ埋めする（`9:5`→`09:05`）。左右非対称を避け表記を安定させるため。
