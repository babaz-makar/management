...
* **staffCode の書式**：英字1文字+数字4桁以上（`/^[A-Za-z]\d{4,}$/`）とする。規定位置(4行目 col2)に無い場合の先頭12行フォールバックで、部署/等級コード（`A1`,`X99` 等の数字1-2桁）を本人コードと誤同定しないための厳格化。フォールバック候補が複数（異なる値）なら黙って先頭を採らず「曖昧」として throw する。staffCode は唯一の安定キーであり、誤ると別人の予定になるため。
* **日付セルの範囲検証**：月 1-12・日 1-31 の範囲外は日付行として扱わずスキップする。`18/45` 等から不正 ISO(`2026-18-45`)を黙って生成しないため。取込は多数行のバッチなので throw ではなく該当行スキップで頑健に弾く。
