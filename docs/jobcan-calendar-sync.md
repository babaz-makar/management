...
* **対象年月ヘッダ: 固定セル(rows[0][0])方式・他セル非走査・無ければ fail-loud throw**：対象年月は `targetMonth`（apps/web はファイル名から渡す本筋）を最優先し、無ければ**先頭セル `rows[0][0]`（タイトルセル）内の最初の `YYYY年M月` のみ**を採用する。**他のセル・他の行は一切見ない**。先頭セルに年月が無ければ throw。実xlsxは `rows[0][0] = "2026年8月"`（単独）で、`度`や後続語 `シフト表` があっても最初のマッチが本物の月。
  ...
  * **セル値の型崩れ耐性**：`normalizeText` は入口で `String(input ?? "")` に正規化する。xlsxライブラリは数値/日付を number/Date 型で返すため、`rows[0][0]=2026`（数値）等でも `text.replace is not a function` でパース全体がクラッシュせず、クリーンに throw または該当行スキップに落ちる（Phase2 結合時に効く防御）。文字列入力の挙動は不変。
  * **恒久策**：Phase2 でファイル名由来の `targetMonth` を必須運用にすれば、in-sheet の年月検出（`rows[0][0]` 依存）自体を不要にでき、タイトルセルの帳票揺れリスクを根絶できる。
