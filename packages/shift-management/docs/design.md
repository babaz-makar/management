...
> 検証（スネイプ／ムーディ）で「実害はないが将来手当てし得る」と整理された事項。
> **いずれも「別人カレンダーへの誤書込」「データ喪失」は起こさない**。可用性・堅牢化に属する。

### 6.1 取り違えトリップワイヤの異種 Unicode 回避（ハードニング＝バックログ）

- **内容:** filename 層の staffCode トリップワイヤ（本章「filename 層は…（3分類）」）は、
  半角 ASCII を前提に `^[A-Z]\d{4}$` で判定する。全角英字（例 `Ｙ0002`）・ゼロ幅スペース（ZWSP）・
  制御文字（NUL 等）を混ぜた staffCode 様トークンは、正規表現に一致しないため
  分類3（`undefined`＝staffCode 情報なし）に倒れ、**小文字混入のような fail-loud 隔離が働かない**。
- **なぜ実害が出ないか:** 書込先を最終的に支配するのはシート内 staffCode（`^[A-Z]\d{4}$`）であり、
  トリップワイヤはあくまで「ファイル名とシートの突合による兆候検知」の補助。回避されても
  別人書込の新規経路は生まれない（トリップワイヤ導入前から在る既存の穴であり、本統一による回帰ではない）。
- **バックログ方針:** 兆候検知の網羅性を上げるなら、判定前に **NFKC 正規化（全角→半角）＋制御文字・ゼロ幅文字の除去**
  を入れて `^[A-Z]\d{4}$` にかける。優先度は低（安全性ではなく検知率の向上）。

### 6.2 付随タグ付きファイル名の誤隔離（安全優先のトレードオフ）

- **内容:** filename 層の分類2は「小文字を含む staffCode 様トークン」を `filename_parse_error` で隔離する。
  そのため、運用者がファイル名に `(v2024)` のような **小文字を含む付随タグ**を付けると、
  それを staffCode 様トークンと見なして誤隔離し得る（＝そのファイルが取り込めない）。
- **トレードオフ:** これは**可用性のみの問題**（取り込めないだけで、誤書込・データ喪失は起きない）。
  「疑わしきは止める（fail-loud）」を優先した結果であり、安全側への意図的な倒し込み。
- **実データでの確認:** 実ジョブカン形式のファイル名（丸括弧が大文字 staffCode 1個のみ。
  例 `山田太郎(A0187) 2026年08月度.xlsx`）ではこの誤隔離は発生しない（スパイクで確認済み）。
  運用者が小文字を含むタグを命名規則に足す場合のみ注意（env 実データ E2E で確認する）。
