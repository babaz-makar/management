 ...
 *
 * 対象メンバーは**チャンネル単位**で明示的に登録する。Bot をチャンネルに招待した
 * タイミングで参加者から選び、あとから参加した人はボタン1つで追加できる。
 */
...
/**
 * チャンネルに登録された対象メンバー1人分（連携前の人も含む）。
 * 連携状況を見て「カレンダー承認をお願いする」判断に使う。
 */
export interface ChannelMember {
  channelId: string;
  slackUserId: string;
  /** Google Calendar 連携済みか（tokens テーブルに行があるか） */
  connected: boolean;
  /** 本人の希望で止めているか。既定 true */
