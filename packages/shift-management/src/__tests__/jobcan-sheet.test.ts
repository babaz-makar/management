...
describe("parseJobcanSheet: 対象年月の決定", () => {
  ...
  it("後続語つきヘッダ(2026年8月度 シフト表)を rows[0][0] から正しく解決する", () => {
    const rows = [["8/1(土)", "", "", "", "", "9:00", "18:00"]];
    // MONTH_HEADER は "2026年8月度 シフト表"。度・後続語があっても最初のマッチ=2026-08
    const entries = parse(rows);
    expect(entries[0].sourceMonth).toBe("2026-08");
  });

  it("rows[0][0] に年月が無ければ throw(固定セル方式・fail-loud)", () => {
    const rows: string[][] = [
      ["シフト表", "", "", "", "", "", ""],
      ["", "", "", "", "", "", ""],
      ["", "", "", "", "", "", ""],
  ...
  it("④: 後段の行/別セルに別年月(2025年12月)が紛れても rows[0][0] の月に固定(hijack消滅)", () => {
    // データ領域・2行目・別セルのどこに別月があっても他セルは一切見ない
    const rows: string[][] = [
      [MONTH_HEADER, "2025年12月", "", "", "", "", ""],
      ["2025年12月", "", "", "", "", "", ""],
      ["", "", "", "", "", "", ""],
      IDENTITY_ROW,
      ["", "", "", "", "", "", ""],
