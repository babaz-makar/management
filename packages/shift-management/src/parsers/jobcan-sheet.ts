...
/**
 * タイトルセル rows[0][0] が "YYYY年M月" で始まる時だけ対象年月を読む(先頭アンカー)。
 * 先頭に別年月/出力日時などが前置されていると silent に別月へ化けるため、
 * 先頭(先行空白のみ許容)から年月で始まらなければ不採用にして fail-loud にする。
 */
const MONTH_HEADER_RE = /^\s*(\d{4})年\s*(\d{1,2})月/;
/**
 * スタッフコード書式。大文字英字1文字+数字ちょうど4桁(社長確認で確定)。
 * ジョブカン由来コードは常に大文字始まりのため、小文字は不正として弾く
 * (staff-directory/allowlist の ^[A-Z]\d{4}$ と統一)。自動で大文字化しない。
 * 部署/チーム/等級コード(例 "A1","X99")や桁数の異なる想定外コードも弾き、
 * 誤同定・別人ひも付けを防ぐ。書式外は取込中止(throw)で fail-loud に気づける。
 */
